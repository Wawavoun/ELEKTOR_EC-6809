...
INIACI  LDA     #$03                    ; F29B: 86 03           RESET ACIA CODE
        ...
        LDA     #$11                    ; F2A0: 86 11           SET CONTROL 8N2 DIV 16
...
ESC_z   JSR     INICRT                  ; F366: BD F2 E9        GO TO 6845 INIT
        ...
        LDA     #$49                    ; F37A: 86 49           SELECT SERIAL SPEED IN TABLE FOR VIA
        JSR     VIAINI                  ; F37C: BD F6 90       
        JMP     ESC_E                   ; F37F: 7E F6 3E        GO TO CLEAR SCREEN AND CURSOR TO HOME

; ?? MAY BE WARM
INIT    BSR     ESC_z                   ; F382: 8D E2           GO TO SCREEN AND VIA INIT
...
ZF3E7   ORA     MDEA5                   ; F3E7: BA DE A5       
        ...
        CMPB    #$4F                    ; F3F1: C1 4F           COMPARE TO 79
...
; CHAR MGNT AFTER CODE ESC Y
; "ESC Y LIN ROW" MOVE CURSOR TO POSITION LIN ROW
; HERE LIN MANAGEMENT
ESC_Y   PSHS    D                       ; F54B: 34 06          
        SUBA    #$20                    ; F54D: 80 20           ASCII CODE TO HEX NUMBER
        CMPA    #$18                    ; F54F: 81 18           COMPARE TO 24
...
; CHAR MGNT AFTER CODE ESC Y + 1 CHAR
; HERE ROW MANAGEMENT AND END OF COMMAND
MF55E   PSHS    X,D                     ; F55E: 34 16          
        SUBA    #$20                    ; F560: 80 20           ASCII CODE TO HEX NUMBER
        CMPA    #$50                    ; F562: 81 50           COMPARE TO 80
...
; COME HERE IF LAST SCREEN LINE SHOULD BE DELETED
...
; COME HERE IF OTHER LINE THAN SCREEN LAST ONE SHOULD BE DELETED
...
; DELETE SCREEN AND CURSOR TO HOME CODE ESC E
...
; CHAR MGNT AFTER ESC x
; COME HERE AFTER ESC x ALREADY RECEIVED
; ?? MOVE CURSOR TO LINE X
ESC_x   PSHS    D                       ; F6AE: 34 06          
        CMPA    #$34                    ; F6B0: 81 34           COMPARE TO "4"
        BEQ     ZF6E6                   ; F6B2: 27 32          
        CMPA    #$35                    ; F6B4: 81 35           COMPARE TO "5"
        BNE     ZF6C1                   ; F6B6: 26 09          
        LDB     MDEA8                   ; F6B8: F6 DE A8       
        ANDB    #$9F                    ; F6BB: C4 9F          
        ORB     #$20                    ; F6BD: CA 20          
        BRA     ZF6F2                   ; F6BF: 20 31          
ZF6C1   CMPA    #$33                    ; F6C1: 81 33           COMPARE TO "3"
        BNE     ZF6FA                   ; F6C3: 26 35          
        STA     MDEA9                   ; F6C5: B7 DE A9       
        BRA     ZF6FA                   ; F6C8: 20 30          

; CHAR MGNT AFTER ESC y
; COME HERE AFTER ESC y ALREADY RECEIVED
; ?? MOVE CURSOR TO ROW Y
ESC_y   PSHS    D                       ; F6CA: 34 06          
        CMPA    #$35                    ; F6CC: 81 35           COMPARE TO "5"
        BEQ     ZF6ED                   ; F6CE: 27 1D          
        CMPA    #$34                    ; F6D0: 81 34           COMPARE TO "4"
        BNE     ZF6DD                   ; F6D2: 26 09          
        LDB     MDEA8                   ; F6D4: F6 DE A8       
        ANDB    #$60                    ; F6D7: C4 60          
        ORB     #$08                    ; F6D9: CA 08          
        BRA     ZF6F2                   ; F6DB: 20 15          
ZF6DD   CMPA    #$33                    ; F6DD: 81 33           COMPARE TO "3"
...
CHRSCR  PSHS    X,D                     ; F702: 34 16          
        BITA    #$60                    ; F704: 85 60           A BIT TO BIT AND WITH $60
        BNE     ZF70D                   ; F706: 26 05           LOW CASE CHAR - GO DIRECTLY TO OUTPUT
        TST     MDEA9                   ; F708: 7D DE A9       
        BEQ     ZF712                   ; F70B: 27 05           JUMP TO CHAR FILTERING ONLY IF MDEA9 = 0
ZF70D   JSR     ZF354                   ; F70D: BD F3 54        NORMAL CHAR JUMP TO PUT IT IN VIDEO RAM
        PULS    PC,X,D                  ; F710: 35 96          

; COME HERE IF CHAR UPPER CASE OR IF MDEA9 = 0
; FILTER SPECIAL CHAR AND DETECT ESC CHAR
...
; COMME HERE IF ESC DETECTED THEN CHANGE GRAPHIC ROUTINE ADDRESS
...
; THESE CODES MUST BE FOLLOWED BY NUMERICAL ARGUMENTS
ESCCOD  PSHS    X,D                     ; F748: 34 16          
        CMPA    #$59                    ; F74A: 81 59           COMPARE WITH CHAR Y / CHANGE CURSOR POSITION
        BNE     ZF753                   ; F74C: 26 05          
        LDD     #ESC_Y                  ; F74E: CC F5 4B       
        BRA     ZF743                   ; F751: 20 F0          
ZF753   CMPA    #$72                    ; F753: 81 72           COMPARE WITH CHAR r / ?? MAY BE CHANGE RS232 SPEED
