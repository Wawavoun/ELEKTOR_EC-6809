...
uint8_t kbd_read()
{
  ...
  switch (result)
    ...
    case 0x1A: result = 'w'; break;
      ...
      // note that caps lock only used on a-z (PhR -> faux en version azerty du clavier mais c'est plus pratique si on travaille en majuscule)
    case 0x41: result = (kbd_shift)? '.' : ';'; break;
    case 0x49: result = (kbd_shift)? '/' : ':'; break;
    case 0x4A: if (kbd_extend) result = '/' ; else result = (kbd_shift)? '§' : '!'; break;
    case 0x54: result = (kbd_shift)? '¨' : '^'; break;
    case 0x5B: if (kbd_altgr) result = '¤' ; else result = (kbd_shift)? '£' : '$' ; break;
    case 0x4E: if (kbd_altgr) result = ']' ; else result = (kbd_shift)? '°' : ')' ; break;
    case 0x55: if (kbd_altgr) result = '}' ; else result = (kbd_shift)? '+' : '=' ; break;
    case 0x3A: result = (kbd_shift)? '?' : ','; break;
    case 0x29: result = ' '; break;
      
    case 0x45: if (kbd_altgr) result = '@' ; else result = (kbd_shift)? '0' : 'à' ; break;
    case 0x16: result = (kbd_shift)? '1' : '&'; break;
    case 0x1E: if (kbd_altgr) result = '~' ; else result = (kbd_shift)? '2' : 'é' ; break;
    case 0x26: if (kbd_altgr) result = '#' ; else result = (kbd_shift)? '3' : '"' ; break;
    case 0x25: if (kbd_altgr) result = '{' ; else result = (kbd_shift)? '4' : 0x27 ; break;
    case 0x2E: if (kbd_altgr) result = '[' ; else result = (kbd_shift)? '5' : '(' ; break;
    case 0x36: if (kbd_altgr) result = '|' ; else result = (kbd_shift)? '6' : '-' ; break;
    case 0x3D: if (kbd_altgr) result = 0x60 ; else result = (kbd_shift)? '7' : 'è' ; break;
    case 0x3E: if (kbd_altgr) result = 0x5C ; else result = (kbd_shift)? '8' : '_' ; break;
    case 0x46: if (kbd_altgr) result = '^' ; else result = (kbd_shift)? '9' : 'ç' ; break;
    
    case 0x0E: result = '²' ; break; // touche "au carré" sur clavier fr
    case 0x5D: result = (kbd_shift)? 'µ' : '*'; break;
    case 0x61: result = (kbd_shift)? '>' : '<'; break;
    case 0x52: result = (kbd_shift)? '%' : 'ù'; break;
