...
static volatile uint8_t kbd_altgr;  
...
ISR(INT0_vect)
{
  ...
  if(kbd_bit_count == 11)
	...
	case 0x11: // alt key (right alt is extended 0x11) si code étendu alors AltGr sinon Alt
	  {
		if (!kbd_extend)
			{
			kbd_alt = kbd_release? FALSE : TRUE;
			if (kbd_altgr) kbd_altgr = FALSE;
			kbd_release = FALSE;
			}
		else
			{
 ...
 * keys between calls to this method, only the later one will be available. Once
...
uint8_t kbd_read()
{
  ...
  switch (result)
    ...
    case 0x4C: result = 'm'; break;
    case 0x31: result = 'n'; break;
    case 0x44: result = 'o'; break;
    case 0x4D: result = 'p'; break;
    case 0x15: result = 'a'; break;
    case 0x2D: result = 'r'; break;
    case 0x1B: result = 's'; break;
    case 0x2C: result = 't'; break;
    case 0x3C: result = 'u'; break;
    case 0x2A: result = 'v'; break;
    case 0x1D: result = 'z'; break;
    case 0x22: result = 'x'; break;
    case 0x35: result = 'y'; break;
    case 0x1A: result = 'w'; break;
      
      // note that caps lock only used on a-z (faux en version azerty du clavier)
    case 0x41: result = (kbd_shift ^ kbd_caps_lock)? '.' : ';'; break;
    case 0x49: result = (kbd_shift ^ kbd_caps_lock)? '/' : ':'; break;
    case 0x4A: if (kbd_extend) result = '/' ; else result = (kbd_shift ^ kbd_caps_lock)? '§' : '!'; break;
    case 0x54: result = (kbd_shift ^ kbd_caps_lock)? '¨' : '^'; break;
    case 0x5B: if (kbd_altgr) result = '¤' ; else result = (kbd_shift ^ kbd_caps_lock)? '£' : '$' ; break;
    case 0x4E: if (kbd_altgr) result = ']' ; else result = (kbd_shift ^ kbd_caps_lock)? '°' : ')' ; break;
    case 0x55: if (kbd_altgr) result = '}' ; else result = (kbd_shift ^ kbd_caps_lock)? '+' : '=' ; break;
    case 0x3A: result = (kbd_shift ^ kbd_caps_lock)? '?' : ','; break;
    case 0x29: result = ' '; break;
      
    case 0x45: if (kbd_altgr) result = '@' ; else result = (kbd_shift ^ kbd_caps_lock)? '0' : 'à' ; break;
    case 0x16: result = (kbd_shift ^ kbd_caps_lock)? '1' : '&'; break;
    case 0x1E: if (kbd_altgr) result = '~' ; else result = (kbd_shift ^ kbd_caps_lock)? '2' : 'é' ; break;
    case 0x26: if (kbd_altgr) result = '#' ; else result = (kbd_shift ^ kbd_caps_lock)? '3' : '"' ; break;
    case 0x25: if (kbd_altgr) result = '{' ; else result = (kbd_shift ^ kbd_caps_lock)? '4' : 0x27 ; break;
    case 0x2E: if (kbd_altgr) result = '[' ; else result = (kbd_shift ^ kbd_caps_lock)? '5' : '(' ; break;
    case 0x36: if (kbd_altgr) result = '|' ; else result = (kbd_shift ^ kbd_caps_lock)? '6' : '-' ; break;
    case 0x3D: if (kbd_altgr) result = 0x60 ; else result = (kbd_shift ^ kbd_caps_lock)? '7' : 'è' ; break;
    case 0x3E: if (kbd_altgr) result = 0x5C ; else result = (kbd_shift ^ kbd_caps_lock)? '8' : '_' ; break;
    case 0x46: if (kbd_altgr) result = '^' ; else result = (kbd_shift ^ kbd_caps_lock)? '9' : 'ç' ; break;
    
    case 0x0E: result = '²' ; break; // touche "au carré" sur clavier fr
    case 0x5D: result = (kbd_shift ^ kbd_caps_lock)? 'µ' : '*'; break;
    case 0x61: result = (kbd_shift ^ kbd_caps_lock)? '>' : '<'; break;
    case 0x52: result = (kbd_shift ^ kbd_caps_lock)? '%' : 'ù'; break;
	
    case 0x0D: result = 0x09; break;
    ...
    case 0x76: result = (esc_mode) ? 0x1B : PS2_KC_ESC ; break; /* Send 0x1B if esc_mode = 1 and 0x8B if esc_mode = 0 */
